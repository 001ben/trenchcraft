...
Open the bucket so its teeth point into the soil, lower to penetrate, then curl and pull the arm in. Tooth-first movement breaks ground even when the mouth faces down; lowering the back of a tightly curled bucket meets resistance. Freshly cut dirt stays loose in the active cut instead of immediately filling the same hole. The bucket holds **0.22 m³**, with a visible soil mound and a percentage indicator. Lift it clear, swing toward the amber spoil strip and open it: the clods slide out of the mouth under gravity, land, settle and become part of the ground. A tipped bucket continues emptying after you release the stick. Using several attachment movements together shares hydraulic speed, and a full bucket lifts slightly more slowly.
...
This is a controls-familiarity game, not machine certification or an excavation safety simulator. It uses kinematic joints with ramped rates, a 25 cm soil height grid, approximate tooth contact and a bounded particle soil (at most 6,000 clods, with settled earth returned to the grid). Clods collide with the bucket shell and the ground but not with the machine body or tracks. It does not model real hydraulic forces, undercarriage contact physics, bank collapse ahead of the teeth or underground services. Tooth-first motion cuts while lowering, curling or crowding; resistance remains a simplified motion limit. Pending clods beside the lip can temporarily count toward bucket load before entering the bowl. Cab visibility and depth judgement need real user feedback. Test ergonomics and GPU performance on an actual phone/iPad before calling them proven.
...
See [DIGGING.md](DIGGING.md) for the contact research, reproduced stalls and remaining physics boundaries, [DESIGN.md](DESIGN.md) for the intended next passes, and [VERIFICATION.md](VERIFICATION.md) for checked behavior.
